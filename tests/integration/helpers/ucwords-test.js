...
module('Integration | Helper | ucwords', function(hooks) {
	setupRenderingTest(hooks);
	test('It formats the input text with `ucwords` format', async function(assert) {
		assert.expect(2);
		this.set('input', 'this Is some TEXT');
		await render(hbs`{{ucwords input}}`);
